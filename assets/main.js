...
/**
 * SidebarMenu - Logica Menu Hamburguesa
 */
...
class SidebarMenu extends HTMLElement {
  constructor() {
    ...
    this.triggerButton = document.querySelector('.sidebar-menu-trigger__button');
    this.drawer = this.querySelector('.sidebar-menu__drawer');
    this.overlay = this.querySelector('.sidebar-menu__overlay');
    this.closeButton = this.querySelector('.js-close-sidebar');
    this.nav = this.querySelector('.sidebar-nav');

    // Estados internos - Independientes del MainMenu
    this.isOpen = false;
    this.childNavOpen = false;
    this.currentActiveSubmenu = null;
    this.opener = null; // NUEVO: Referencia al elemento que abrió el submenú

    // Configuración
    this.menuSensitivity = parseInt(this.dataset.menuSensitivity) || 200;

    this.init();
  }

  /**
   * Inicialización - No depende de breakpoints, siempre funciona igual
   */
  init() {
    if (!this.triggerButton || !this.drawer) return;
  ...
  /**
   * Event Listeners - Gestiona todos los eventos del sidebar
   */
  addEventListeners() {
    // Trigger button para abrir el sidebar
   ...
   */
  setupAccessibility() {
    // Configurar aria-controls
    if (this.triggerButton && this.drawer) {
      this.triggerButton.setAttribute('aria-controls', this.drawer.id);
    }
  }

  /**
   * Maneja el click del trigger button
   */
  ...
  openSidebar() {
    ...
    // Trap focus - REUTILIZA función existente
    trapFocus(this.drawer);

    // Pausar medios - REUTILIZA función existente  
    window.pauseAllMedia && window.pauseAllMedia();

    // Dispatch evento personalizado
    this.dispatchEvent(new CustomEvent('sidebar:opened', { bubbles: true }));
  }
  ...
  closeSidebar() {
    ...
    // Cerrar cualquier submenú abierto primero
    this.closeAllSubmenus();

    // Iniciar cierre con transición
    this.drawer.classList.add('is-closing');
    this.drawer.classList.remove('is-open');

    // El resto se maneja en handleTransitionEnd
  }

  /**
   * Maneja el final de las transiciones
   */
  handleTransitionEnd(evt) {
    if (evt.target !== this.drawer) return;
  ...
  finishClosing() {
    ...
    // Remover trap focus - REUTILIZA función existente
    removeTrapFocus(this.triggerButton);
  ...
  /**
   * Maneja la navegación interna - ADAPTADA de MainMenu.handleNavClick
   * Incluye lógica de botones "Atrás" y navegación multinivel
   */
  handleNavClick(evt) {
    let target = evt.target;

    // Buscar el elemento relevante
    const toggle = target.closest('.js-sidebar-toggle');
    const backBtn = target.closest('.js-back');
    const detailsElement = target.closest('details');

    // Si es un botón de retroceso
    if (backBtn) {
      evt.preventDefault();
      this.handleBackNavigation(detailsElement);
      return;
    }

    // Si es un link normal, permitir navegación
    if (target.href && target.href !== '#' && !toggle) {
      // Opcionalmente cerrar el sidebar al navegar
      if (this.dataset.closeOnNavigate !== 'false') {
        setTimeout(() => this.closeSidebar(), 100);
      }
      return;
    }

    // Manejar toggles de submenú
    if (toggle) {
      evt.preventDefault();
      this.handleSubmenuToggle(detailsElement, toggle);
    }
  }
   ...
   */
  handleBackNavigation(detailsElement) {
    if (!detailsElement) return;

    // Cerrar el submenú actual con animación
    this.closeSubmenu(detailsElement);

    // Restaurar el foco al elemento padre
    const parentToggle = detailsElement.querySelector('.js-sidebar-toggle');
    if (parentToggle) {
      setTimeout(() => parentToggle.focus(), 300);
    }
  }
 ...
 */
  handleSubmenuToggle(detailsElement, toggle) {
    if (!detailsElement) return;

    const isCurrentlyOpen = detailsElement.classList.contains('is-open');

    if (isCurrentlyOpen) {
      this.closeSubmenu(detailsElement);
    } else {
      // Cerrar otros submenús del mismo nivel
      const parentList = detailsElement.closest('ul');
      const otherOpenMenus = parentList.querySelectorAll('details.is-open');
      otherOpenMenus.forEach(menu => {
        if (menu !== detailsElement) {
          this.closeSubmenu(menu);
        }
      });

      this.openSubmenu(detailsElement);
    }
  }


  /**
 * Abre un submenú con animación
 */
  openSubmenu(detailsElement) {
    ...
    // Aplicar clases y estados con un pequeño delay para la animación
    requestAnimationFrame(() => {
      detailsElement.classList.add('is-open');
      this.childNavOpen = true;
      this.currentActiveSubmenu = detailsElement;

      // Animar la entrada del contenido
      const content = detailsElement.querySelector('.sidebar-nav__child, .sidebar-nav__grandchild-panel');
      if (content) {
    ...
    // Trap focus en el submenú si está disponible
    if (typeof trapFocus !== 'undefined') {
      trapFocus(detailsElement);
    }

    // Manejar megamenús si es necesario
    if (detailsElement.classList.contains('js-sidebar-mega-nav')) {
      this.handleMegaMenuLogic(detailsElement);
    }
  }

  /**
   * Cierra un submenú con animación
   */
  closeSubmenu(detailsElement) {
    ...
    // Completar cierre después de la animación
    setTimeout(() => {
      ...
      if (this.currentActiveSubmenu === detailsElement) {
        this.currentActiveSubmenu = null;
        this.childNavOpen = false;
      }
    }, 200); // Ajustar según la duración de tu transición CSS
  }
 ...
 */
  closeAllSubmenus() {
    const openSubmenus = this.nav.querySelectorAll('details.is-open');
  ...
  addEventListeners() {
    ...
    // Navegación interna - Delegación de eventos
    this.nav?.addEventListener('click', this.handleNavClick.bind(this));
    ...
    // Accesibilidad - Manejar focus
    this.drawer?.addEventListener('focusout', this.handleFocusOut.bind(this));
  }

  /**
   * Maneja lógica específica de megamenús - REUTILIZA funciones existentes
   */
  handleMegaMenuLogic(megaElement) {
    // Aquí puedes reutilizar funciones como handleSidenavMenuToggle
    // si necesitas lógica específica para megamenús en el sidebar

    // Por ejemplo, para megamenús tipo sidebar:
    if (megaElement.querySelector('.sidebar-mega-nav--sidebar')) {
      const firstSummary = megaElement.querySelector('details[open] summary');
      if (firstSummary && typeof MainMenu.handleSidenavMenuToggle === 'function') {
        // REUTILIZA la función de MainMenu
        MainMenu.handleSidenavMenuToggle(null, firstSummary);
      }
    }
  }
  ...
  /**
 * Maneja cuando el foco sale del sidebar
 */
  handleFocusOut(evt) {
    // Solo en desktop, verificar si el foco salió completamente del sidebar
    if (window.matchMedia('(min-width: 769px)').matches) {
      setTimeout(() => {
        if (!this.drawer.contains(document.activeElement) && this.isOpen) {
          // Opcionalmente cerrar el sidebar o devolver el foco
          // this.closeSidebar();
        }
      }, 0);
    }
  ...
  disconnectedCallback() {
    ...
    this.triggerButton?.removeEventListener('click', this.handleTriggerClick.bind(this));
    this.closeButton?.removeEventListener('click', this.closeSidebar.bind(this));
    this.overlay?.removeEventListener('click', this.closeSidebar.bind(this));
    this.nav?.removeEventListener('click', this.handleNavClick.bind(this));
    this.drawer?.removeEventListener('transitionend', this.handleTransitionEnd.bind(this));
    this.drawer?.removeEventListener('focusout', this.handleFocusOut.bind(this));
  }
}
